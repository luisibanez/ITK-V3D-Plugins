...
Q_EXPORT_PLUGIN2(OtsuThreshold, ITKOtsuThresholdPlugin)

QStringList ITKOtsuThresholdPlugin::menulist() const
{
	return QStringList() << QObject::tr("ITK OtsuThreshold ...");
}

template<typename TPixelType>
class ITKOtsuThresholdSpecializaed
{
public:
	void Execute(const QString &menu_name, V3DPluginCallback &callback, QWidget *parent)
	{
		...
		typedef itk::OtsuThresholdImageFilter<ImageType_mid,ImageType_mid> FilterType;
		typename FilterType::Pointer filter = FilterType::New();

		//set paras
		unsigned char outsideValue		=255;
		unsigned char insideValue		=0;
		...
		if (menu_name == QObject::tr("ITK OtsuThreshold ..."))
		{
			ITKOtsuThresholdDialog d(p4DImage, parent);
...
#define EXECUTE( v3d_pixel_type, c_pixel_type ) \
  case v3d_pixel_type: \
    { \
	ITKOtsuThresholdSpecializaed< c_pixel_type > runner; \
...
void ITKOtsuThresholdPlugin::domenu(const QString & menu_name, V3DPluginCallback & callback, QWidget * parent)
{
