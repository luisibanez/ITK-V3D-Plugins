...
Q_EXPORT_PLUGIN2(MinMaxCurvatureFlow, ITKMinMaxCurvatureFlowPlugin)

QStringList ITKMinMaxCurvatureFlowPlugin::menulist() const
{
	return QStringList() << QObject::tr("ITK MinMaxCurvatureFlow Filter ...");
}

template<typename TPixelType>
class ITKMinMaxCurvatureFlowSpecializaed
{
public:
	void Execute(const QString &arg, Image4DSimple *p4DImage, QWidget *parent)
	{
		...
		//------------------------------------------------------------------
		//setup filter: MinMax Curvature Flow
		typedef itk::MinMaxCurvatureFlowImageFilter<ImageType_mid,ImageType_mid> AniFilterType;
		...
		if (arg == QObject::tr("ITK MinMaxCurvatureFlow Filter ..."))
		{
			ITKMinMaxCurvatureFlowDialog d(p4DImage, parent);
...
#define EXECUTE( v3d_pixel_type, c_pixel_type ) \
  case v3d_pixel_type: \
    { \
	  ITKMinMaxCurvatureFlowSpecializaed< c_pixel_type > runner; \
...
void ITKMinMaxCurvatureFlowPlugin::processImage(const QString &arg,
